...
export class DesktopCredentialVault implements CredentialVault {
    async set(provider: string, credential: string): Promise<void> { await invoke('set_provider_credential', { provider, credential }); }
    async get(provider: string): Promise<string | null> { return invoke<string | null>('get_provider_credential', { provider }); }
    async remove(provider: string): Promise<void> { await invoke('delete_provider_credential', { provider }); }
}

export function createCredentialVault(): CredentialVault { return isTauri() ? new DesktopCredentialVault() : new SessionCredentialVault(); }
