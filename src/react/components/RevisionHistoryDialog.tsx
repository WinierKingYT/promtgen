...
export function RevisionHistoryDialog({ open, project, onRestore, onClose }: RevisionHistoryDialogProps) {
  ...
  const defaultFrom = revisions.find((revision: any) => revision.number < project.canonicalRevision)?.id || revisions[0]?.id || 'current';
  ...
  useEffect(() => {
    if (!open) return;
    const nextFrom = revisions.find((revision: any) => revision.number < project.canonicalRevision)?.id || revisions[0]?.id || 'current';
      ...
      <div className="revision-body">
        {revisions.length === 0 ? <div className="empty-history"><History size={25} /><b>Henüz karşılaştırılabilir snapshot yok</b><p>İlk plan değişikliğinden sonra revision geçmişi burada görünecek.</p></div> : <>
          <div className="revision-selectors">
            ...
            <ArrowRight size={18} />
            <label htmlFor="revision-to">Yeni sürüm<select id="revision-to" value={to} onChange={event => setTo(event.target.value)}><option value="current">r{project.canonicalRevision} — Güncel plan</option>{revisions.map((revision: any) => <option key={revision.id} value={revision.id}>r{revision.number} — {revision.summary}</option>)}</select></label>
          </div>
