...
import { beginExecutionSession, buildExecutionPrompt, getNextExecutionRole, proposeExecution, recordExecutionResult } from '../../v4/execution-orchestrator.js';
...
export function ExecutionPanel({ project, onCommit }: { project: any; onCommit: (project: any, message: string) => void }) {
  ...
  useEffect(() => { if (desktop) getExecutionCapabilities().then(setCapabilities).catch((value: unknown) => setError(String(value))); }, [desktop]);
