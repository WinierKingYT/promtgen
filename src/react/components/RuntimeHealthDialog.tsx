...
export function RuntimeHealthDialog({ open, settings, credential, onClose }: RuntimeHealthDialogProps) {
  ...
  return <dialog ref={dialogRef} className="runtime-health-dialog" aria-labelledby="runtime-health-title" onCancel={close} onClose={onClose}>
    ...
    {desktop && <div className="codex-cli-config"><span><Terminal size={17}/><span><b>Codex CLI kaynağı</b><small>{executionCapabilities?.customCodexConfigured ? 'Kullanıcı seçimi aktif (Çalışabilirlik doğrulandı; yayıncı veya binary imzasını doğrulamaz).' : 'Varsayılan PATH algılaması kullanılıyor.'}</small></span></span><div><button type="button" disabled={busy} onClick={chooseCodex}><FolderSearch size={15}/> {executionCapabilities?.customCodexConfigured ? 'Seçimi değiştir' : 'Codex CLI seç'}</button>{executionCapabilities?.customCodexConfigured && <button type="button" disabled={busy} onClick={usePathCodex}><RotateCcw size={15}/> PATH’e dön</button>}</div></div>}
