...
export function ModulePanel({ project, onCommit }: { project: any; onCommit: (project: any, message: string, commandType?: string) => void }) {
  ...
  return <details className="module-panel">
    ...
    <div className="module-body">
      ...
      {activation && <div className="module-preview" role="region" aria-label="Modül aktivasyon önizlemesi"><b>{activation.moduleIds.join(' → ')}</b>{activation.upgrades?.length > 0 && <small>{activation.upgrades.map((item: any) => `${item.id} ${item.fromVersion} → ${item.toVersion}`).join(', ')} yükseltmesi</small>}<small>Zorunlu bölümler: {activation.requiredSections.join(', ') || 'değişiklik yok'}</small>{activation.domainQuestions?.length > 0 && <small>{activation.domainQuestions.length} alan sorusu readiness ve inceleme akışına bağlanacak.</small>}<div><button type="button" onClick={() => setActivation(null)}>Vazgeç</button><button type="button" className="primary" disabled={activation.errors.length > 0} onClick={approve}><Check size={14}/> Modülleri onayla</button></div></div>}
