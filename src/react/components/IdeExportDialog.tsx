...
export function IdeExportDialog({ open, project, onCommit, onClose }: any) {
  ...
  const download = async () => {
    ...
    try {
      ...
      onCommit(next, `Plan sürümü r${result.record.canonicalRevision} için IDE çalışma paketi oluşturuldu.`);
      close();
  ...
  return <dialog ref={dialogRef} className="ide-export-dialog" aria-labelledby="ide-export-title" onCancel={close} onClose={onClose}>
    ...
    <p className="dialog-lead">Seçtiğin plan sürümü; görev bağımlılıkları, doğrulama sözleşmesi ve IDE’nin otomatik okuyacağı talimat dosyalarıyla tek bir ZIP’e dönüştürülür veya panoya kopyalanır.</p>
    <div className="ide-export-body">
      ...
      <section className="ide-file-preview" aria-label="Üretilecek dosyalar">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
          <div><span className="meta">DOSYA ÖNİZLEMESİ VE PANODAN KOPYALAMA</span><b>{fileKeys.length} dosya · plan sürümü r{preview.source.canonicalRevision}</b></div>
