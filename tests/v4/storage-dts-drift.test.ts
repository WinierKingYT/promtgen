...
/**
 * storage.d.ts DRIFT REGISTER
 *
 * `storage.js` (implementation) ve `storage.d.ts` (elle yazılmış sidecar)
 * bugün `tsc` tarafından ASLA birlikte görülmüyor: sidecar tam olarak
 * implementasyonu gölgeliyor. Bu üç madde, storage.js TypeScript'e
 * dönüştürülüp sidecar silindiğinde derleyicinin ilk kez karşılaşacağı
 * gerçek çalışma zamanı davranışını sabitler.
 */

describe('DRIFT #1 — MemoryProjectRepository.migrationBackups şekli (storage.d.ts:17 vs storage.js:317)', () => {
  it('storage.d.ts, migrationBackups değerini `{ projectSnapshot }` olarak bildirir; GERÇEKTE createCheckpoint() sonucunun TAMAMI saklanır', async () => {
    const repository = new MemoryProjectRepository();
    // migrationBackups yalnızca #migrate() içinde migration.migrated === true
    // olduğunda doldurulur (storage.js:312-320). Legacy bir belge (schemaVersion
    // eksik) bunu tetikler. `repository.projects`, storage.d.ts:16'da
    ...
    // storage.d.ts:17 iddiası: Map<string, { projectSnapshot: ProjectDocumentV5 }>
    ...
    assert.deepEqual(
      ...
      'GERÇEK ŞEKİL storage.d.ts:17 bildiriminden DAHA GENİŞ: sadece projectSnapshot değil, createCheckpoint() sonucunun tamamı saklanıyor'
    );
    ...
    assert.ok(backup.projectSnapshot, 'projectSnapshot alanı da mevcut (d.ts kısmen doğru, ama eksik)');
  });
});

describe('DRIFT #2 — IndexedDbProjectRepository.listCheckpoints() checksumAlgorithm (storage.d.ts:11 vs storage.js:106)', () => {
  beforeEach(resetIndexedDbFixture);

  it('storage.d.ts checksumAlgorithm\'i sabit literal \'SHA-256\' olarak bildirir; GERÇEKTE infrastructure/storage/integrity.ts\'ten import edilen INTEGRITY_ALGORITHM sabiti kullanılır (bugün ikisi aynı değeri taşır ama yapısal olarak BAĞLANTISIZ)', async () => {
    ...
    assert.equal(checkpoint.checksumAlgorithm, 'SHA-256', 'BUGÜN d.ts\'nin sabit literal iddiasıyla aynı değeri taşıyor — ama INTEGRITY_ALGORITHM değişirse d.ts sessizce yanlış kalır çünkü ikisi arasında hiçbir tip bağı yok');
  });
});

describe('DRIFT #3 — purge() backupsDeleted asimetrisi: Memory gerçek sayar, IndexedDb HER ZAMAN 0 döner (storage.js:290 vs storage.js:357)', () => {
  beforeEach(resetIndexedDbFixture);
  ...
  it('IndexedDbProjectRepository.purge(): migration yedeği checkpoint deposuna gömülü olduğu için backupsDeleted HER ZAMAN 0 döner — Memory ile AYNI davranmıyor', async () => {
    ...
    // yazmak yerine, storage.js'in migration yedeğini de normal checkpoint
    // deposuna (`buildPersistentCheckpoint(migrationBackup, ...)`,
    // storage.js:195-197,211) yazdığını save() akışı üzerinden gözlemliyoruz:
    ...
    assert.equal(result.backupsDeleted, 0, 'IndexedDbProjectRepository.purge() backupsDeleted alanını asla artırmaz (storage.js:290)');
